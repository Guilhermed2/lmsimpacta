...
class Disciplina(models.Model):
    ...
    def clean(self):
        if self.percentual_teorico > 100 or self.percentual_teorico < 0:
            raise ValidationError('O percentual Teórico está deve estar entre 0 e 100.')

        if self.percentual_pratico > 100 or self.percentual_pratico < 0:
            raise ValidationError('O percentual Prático está deve estar entre 0 e 100.')

        if self.percentual_pratico + self.percentual_teorico != 100:
            raise ValidationError('A soma dos percentuais Teórico e Prático não pode ser maior que 100.')